...
Enabled on all six tables, and **the policies live in `schema.ts`** (`pgPolicy` +
...
Policies: public `SELECT` on `venues`, `categories`, `products` and `venue_tables`, plus an owner-scoped
