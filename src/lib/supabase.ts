...
type VenueRow = {
  ...
  /**
   * How orders are identified, how a table order is served, and whether table
   * mode still asks for a first name. Read through `parseOrderSettings`, which
   * forces the service to `counter` in name mode.
   */
...
type OrderRow = {
  ...
  /** The table the order came from, if it still exists. */
  table_id: string | null
  /** The table's number and label, copied when the order was placed. */
...
/**
 * A table of a venue. Publicly readable: the customer menu's table picker
 * reads it as `anon`. `public_id` is what a table's QR code carries — opaque,
 * but not a secret.
 */
type VenueTableRow = {
  id: string
  venue_id: string
  number: number
  /** « Terrasse », or `null`: the table is simply « Table 12 ». */
  label: string | null
  public_id: string
  created_at: string
  updated_at: string
...
export type Database = {
  public: {
    Tables: {
      ...
      venue_tables: {
        Row: VenueTableRow
        /* `public_id` is generated by the column default, never sent. */
        Insert: Insert<VenueTableRow, Timestamps | 'label' | 'public_id'>
        Update: Pick<Partial<VenueTableRow>, 'number' | 'label'>
        Relationships: []
      }
      categories: {
        Row: CategoryRow
    ...
    Functions: {
      ...
      place_order: {
        Args: {
          venue_slug: string
          /** Required in name mode; `null` or optional in table mode. */
          guest_name: string | null
          guest_note: string | null
          items: Array<{ product_id: string; quantity: number }>
          /**
           * The table's public id, in table mode (migration CLOCLO-32).
           * Defaulted in SQL, so a caller that predates tables can omit it.
           */
          guest_table?: string | null
        }
